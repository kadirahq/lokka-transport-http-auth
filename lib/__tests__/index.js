...
describe('LokkaHTTPAuthTransport', () => {
  describe('constructor()', () => {
    ...
    describe('with a proper auth url', () => {
      it('should get the proper auth headers', () => {
        ...
        const options = transport._transport._buildOptions({aa: 10});
      ...
      it('should get the proper endpoint', () => {
        const url = 'http://u:p@localhost/path';
        const transport = new LokkaHTTPAuthTransport(url);
        expect(transport._transport.endpoint).to.be.equal('http://localhost:80/path');
      });
    });
    ...
    describe('without a port for https', () => {
      it('should get port 443', () => {
        ...
        const transport = new LokkaHTTPAuthTransport(url);
        expect(transport._transport.endpoint).to.be.equal('https://localhost:443/path');
      });
    });
  ...
  describe('send()', () => {
    it('should call original .send() method', async () => {
      const url = 'http://u:p@localhost/path';
      const transport = new LokkaHTTPAuthTransport(url);
      const data = {aa: 10};
      const q = `{items}`;
      transport._transport.send = query => {
        expect(query).to.be.equal(q);
        return Promise.resolve(data);
      };

      const response = await transport.send(q);
